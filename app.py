...
st.set_page_config(page_title="IA TERMINAL ELITE V3.1", layout="wide")
...
if 'hist_cerrado' not in st.session_state: st.session_state.hist_cerrado = [{'PNL': '0.1%'}] # Base para la barra
...
with col_head1:
    st.markdown(f"""
        <div style="text-align: center; color: white;">
            <span style="font-size: 22px; font-weight: bold;">🛰️ TERMINAL IA ELITE | RENDIMIENTO</span>
            <div class="perf-bar">
                <div style="background: #3fb950; width: {p_green}%;"></div>
                <div style="background: #f85149; width: {100-p_green}%;"></div>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 12px; font-weight: bold;">
                <span style="color: #3fb950;">WIN: {p_green:.1f}%</span>
                <span style="color: #f85149;">LOSS: {100-p_green:.1f}%</span>
            </div>
        </div>
        """, unsafe_allow_html=True)

with col_head2:
    st.markdown('<div style="background:white; padding:5px; border-radius:5px; text-align:center;"><img src="https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=AUDIT_DATA" width="80"></div>', unsafe_allow_html=True)

st.markdown(f'<div class="ia-log">🧠 PENSAMIENTO IA: Verificando disparadores de mercado y protegiendo capital...</div>', unsafe_allow_html=True)
...
try:
    ...
    for i, sym in enumerate(selected):
        if sym in tickers:
            ...
            # --- REINCORPORACIÓN DE DISPARADORES Y ESTRATEGIAS ---
            if "VEREM" in s_name: trig, strat = "IMPULSO", "AGRESIVO"
            elif "BTC" in s_name: trig, strat = "INSTITUCIONAL", "EXPERTO"
            elif "ETH" in s_name: trig, strat = "BALLENAS", "IA EXPERTO"
            else: trig, strat = "PATRÓN FIBO", "SCALPING"
            ...
            with cols[i]:
                ...
                <div class="card-tv">
                    ...
                    <div class="exec-grid">
                        <div style="color:white; font-size:10px;"><span style="color:#888;font-size:7px;">IN</span><br>{sig['e']:.2f}</div>
